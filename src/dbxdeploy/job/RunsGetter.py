...
class RunsGetter:
    ...
    def get(self, notebook_path: PurePosixPath, cluster_id: str, package_metadata: PackageMetadata):
        notebook_runs = []
        ...
        while True:
            ...
            if "runs" in response:
                new_runs = list(filter(lambda run: self.__filter_run(run, notebook_path, cluster_id, package_metadata), response["runs"]))
    ...
    def __filter_run(self, run: dict, notebook_path: PurePosixPath, cluster_id: str, package_metadata: PackageMetadata):
        reg_ex = package_metadata.get_notebook_path_reg_ex(self.__workspace_base_dir, notebook_path)

        if "notebook_task" not in run["task"]:
            return False

        return re.match(reg_ex, run["task"]["notebook_task"]["notebook_path"]) and run["cluster_spec"]["existing_cluster_id"] == cluster_id
